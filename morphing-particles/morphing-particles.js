...
(function () {
  ...
  /* ── Worker source (jittered grid + dark-pixel scan, off main thread) ── */
  var WORKER_SRC = [
    'self.onmessage = function(e) {',
    '  var pixels = new Uint8Array(e.data.pixels);',
    '  var density = e.data.density;',
    '',
    '  /* Grid spacing: density 0→300 maps to spacing 20→4 */',
    '  var spacing = 20 - (density / 300) * 16;',
    '  if (spacing < 4) spacing = 4;',
    '  var cols = Math.floor(500 / spacing);',
    '  var rows = Math.floor(500 / spacing);',
    '  var count = cols * rows;',
    '  var baseFlat = new Float32Array(count * 2);',
    '  var targetFlat = new Float32Array(count * 2);',
    '',
    '  /* Brightness at pixel (0=black, 1=white) */',
    '  function bri(x, y) {',
    '    if (x < 0 || x >= 500 || y < 0 || y >= 500) return 1;',
    '    return pixels[(x + y * 500) * 4] / 255;',
    '  }',
    '',
    '  /* Search radius for nearest dark pixel */',
    '  var R = 50;',
    '',
    ...
    '',
    '      /* Base point: grid cell center + random jitter */',
    '      var bx = (col + 0.5) * spacing + (Math.random() - 0.5) * spacing * 0.8;',
    '      var by = (row + 0.5) * spacing + (Math.random() - 0.5) * spacing * 0.8;',
    '      if (bx < 0) bx = 0; if (bx > 499) bx = 499;',
    '      if (by < 0) by = 0; if (by > 499) by = 499;',
    '      baseFlat[idx * 2] = bx;',
    '      baseFlat[idx * 2 + 1] = by;',
    '',
    '      /* Target: find nearest dark pixel within radius R */',
    '      var bestD = Infinity;',
    '      var tx = bx - 250, ty = by - 250;',
    '      var ibx = Math.round(bx), iby = Math.round(by);',
    '      var x0 = ibx - R, x1 = ibx + R, y0 = iby - R, y1 = iby + R;',
    '      if (x0 < 0) x0 = 0; if (x1 > 499) x1 = 499;',
    '      if (y0 < 0) y0 = 0; if (y1 > 499) y1 = 499;',
    '',
    '      /* Step by 2 to cut iterations by 4x — still sub-pixel accurate enough */',
    '      for (var sy = y0; sy <= y1; sy += 2) {',
    '        for (var sx = x0; sx <= x1; sx += 2) {',
    '          if (bri(sx, sy) > 0.5) continue;',
    '          var dx = sx - bx, dy = sy - by;',
    '          var d = dx * dx + dy * dy;',
    '          if (d < bestD) { bestD = d; tx = sx - 250; ty = sy - 250; }',
    '        }',
    '      }',
    '',
    '      targetFlat[idx * 2] = tx;',
    '      targetFlat[idx * 2 + 1] = ty;',
    '    }',
    '  }',
    '',
    '  self.postMessage(',
    '    { baseFlat: baseFlat.buffer, targetFlat: targetFlat.buffer, count: count },',
    '    [baseFlat.buffer, targetFlat.buffer]',
    '  );',
    '};'
  ].join('\n');

  /* ── Run sampling in a Web Worker ───────────────────────────────────── */
  function computeParticlesAsync(imgCanvas, density) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      worker.postMessage(
        { pixels: pixels.buffer, density: density },
        [pixels.buffer]
      );
    });
  }
